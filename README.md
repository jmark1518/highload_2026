...
| **Интеграции и ИИ** | ЭМК, лаборатории, страховые, внешний ИИ-сервис | Средний RPS, асинхронный | Некритичный |

**Обоснование.** Клиентский API обрабатывает ~90% запросов, но не хранит данные. Домен данных — stateful-нагрузка, требующая консистентности. Домен интеграций — «мост» к внешним системам.
...
GSLB возвращает IP ближайшего доступного ДЦ на основе: геолокации (GeoDNS), задержки, доступности (health checks), веса ДЦ.

**Схема:** пользователь → DNS-запрос `api.emias-clone.ru` → GSLB-кластер (Anycast DNS) → правила: Москва→ДЦ1, СПб→ДЦ2, Сибирь→ДЦ3, fallback→ДЦ1 → ДЦ-1 (60%), ДЦ-2 (25%), ДЦ-3 (15%).

**Возможная реализация:**
- Health checks: опрос `/health` каждые 5 секунд; 3 фейла (15 секунд) → исключение из DNS.
- TTL DNS: 30 секунд.
...
*ДЦ-1 недоступен:* health check не проходит 3 раза (15 секунд), ДЦ-1 исключается из GSLB, веса: ДЦ-2 = 55%, ДЦ-3 = 30%, ДЦ-4 = 15%. Anycast-узел в Москве перестаёт анонсировать IP.
